...
const expectedIds = [
  ...
  'nanami-meme-005', 'nanami-meme-007', 'nanami-meme-008',
] as const

describe('curated Nanami archive', () => {
  it('preserves all 13 photo and 6 meme IDs in source order', () => {
    ...
    expect(archiveItems.map(({ order }) => order)).toEqual([
      1, 2, 3, 4, 5, 6, 7, 8, 9, 12, 14, 15, 16, 17, 18, 20, 21, 23, 24,
    ])
  })
  ...
  it('uses explicit metadata-free responsive source pairs', () => {
    for (const item of archiveItems) {
      ...
      expect(item.captureDate).toBeUndefined()
  ...
  it('derives primary and portrait collections from real memberships', () => {
    ...
    expect(portraits.map(({ id }) => id)).toEqual(['nanami-photo-003', 'nanami-photo-006'])
    expect(portraits.length).toBeGreaterThan(0)
    expect(portraits.length).toBeLessThan(13)
