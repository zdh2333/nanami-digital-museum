...
export function App() {
  ...
  const [isAdmin, setIsAdmin] = useState(() => window.location.hash === '#admin' || window.location.pathname === '/admin')

  useEffect(() => {
    const syncAdminRoute = () => setIsAdmin(window.location.hash === '#admin' || window.location.pathname === '/admin')
